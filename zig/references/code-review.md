...
| `return &local_var` | Compile error (0.16.0) | Returning address of expired local variable |
...
| `@Type(info)` | `@Int`/`@Struct`/`@Union`/`@Enum`/`@Pointer`/`@Fn`/`@Tuple`/`@EnumLiteral` | Compile error |
| `@cImport`/`@cInclude` | `b.addTranslateC()` in build.zig | Compile error (deprecated) |
| `@intFromFloat` | `@floor`/`@ceil`/`@round`/`@trunc` to integer | Compile error (deprecated) |
| Runtime `vector[i]` | Coerce vector to array first | Compile error |
| `return &local_var` | Return by value or heap allocate | Compile error |
| Pointer in `packed struct`/`packed union` | Use `usize` with `@ptrFromInt` | Compile error |
| Implicit backing type in `extern` | Explicit `enum(u8)` / `packed struct(u8)` | Compile error |
...
**Preconditions:**
- [ ] Code uses 0.16.0 `std.Io.Writer` API
