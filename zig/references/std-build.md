...
> `exe.addIncludePath()`, `exe.linkLibC()` are **deprecated**.
> Use `exe.root_module.*` equivalents shown below.

### C Translation (0.16.0)
`@cImport` is deprecated. Use `b.addTranslateC()` in the build system instead.

```zig
// Create a C header file (e.g., src/my_c_api.h)
const c_translate = b.addTranslateC(.{
    .root_source_file = b.path("src/my_c_api.h"),
    .target = target,
    .optimize = optimize,
});

// Add as a module import
exe.root_module.addImport("c_api", c_translate.createModule());
