...
### @intFromFloat (DEPRECATED in 0.16.0)
```zig
@intFromFloat(value: anytype) anytype
```
**Deprecated:** Use `@floor`, `@ceil`, `@round`, or `@trunc` directly to integer instead.

Before:
```zig
const i: i32 = @intFromFloat(@floor(value));
```
After:
```zig
const i: i32 = @floor(value);
```
...
### @Type - REMOVED (0.16.0)
Replaced with individual type-creating builtins: `@Int`, `@Enum`, `@Struct`, `@Union`, `@Pointer`, `@Fn`, `@Tuple`, `@EnumLiteral`.
...
// Create a tuple type
const Tuple = @Tuple(&.{ u32, u8 });

// Create an enum literal
const lit = @EnumLiteral("name"); // usually just use .name
```
...
### @cImport (DEPRECATED in 0.16.0)
```zig
@cImport(expr) type
```
**Deprecated:** Use `b.addTranslateC()` in the build system instead.
